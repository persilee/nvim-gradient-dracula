...
function M.get()
	return {
		...
		LspInlayHint = { fg = c.comment, bg = c.none, italic = true },

		-- Native Completion menu

		-- Virtual text inserted when hovering a completion item
		ComplMatchIns = { fg = c.comment, bg = c.none, italic = true },
		Pmenu = { bg = c.bg_darker },
		PmenuSel = { bg = c.selection },
		PmenuThumb = { bg = c.bg_light },
		-- Color of matched text in completion menu
		PmenuMatch = { fg = c.cyan },
		-- Selected completion item
		PmenuMatchSel = { fg = c.cyan, bold = true },
