...
function M.get(config)
	return {
		...
		-- glowing active line number (discrete take on the VSCode glow effect)
		CursorLineNr = { fg = c.rainbow[1], bg = c.none, bold = true },
		MatchParen = { fg = c.scale.func[11], bg = c.bg_lighter, bold = true },
		...
		FloatBorder = { fg = c.scale.type[10], bg = c.none },
		...
		StatusLine = { fg = c.cyan, bg = bg(config.transparent, c.none, c.bg_dark) },
		StatusLineNC = { fg = c.comment, bg = bg(config.transparent, c.none, c.bg_dark), bold = false },
